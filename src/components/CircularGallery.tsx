...
class App {
  ...
  onWheel(e: any) {
    ...
    const itemWidth = this.medias[0]?.width || this.scrollSpeed;
    this.scroll.target += (delta > 0 ? itemWidth : -itemWidth);
  ...
  onCheck() {
    ...
    const actualIndex = itemIndex % (this.mediasImages.length / 2);
    if (this.onChange) {
      this.onChange(actualIndex);
    }
  }
  onResize() {
    this.screen = {
